...
import schemas.administrator.tables.daos.RoUsersDao;
...
public class RoUtilisateursImpl extends RoUsersDao {
	...
	public String findPosteComptable(String utilisateur) {
		...
		//RoUsers roUtilisateur = fetchOne(Tables.RO_USERS.USER, (utilisateur.split("@")[0]).toUpperCase());	
		...
		//System.out.println("yyyyyy" + roUtilisateur.getUser());
 		...
 		if(roUtilisateur!=null) posteComptable = roUtilisateur.getComptable();
	...
	public RoUsersRecord getRoUtilisateursByUtilisateur(String utilisateur){
		List<RoUsersRecord> roUtilisateursRecords = sqlContext.selectFrom(Tables.RO_USERS)
																.where(Tables.RO_USERS.USER.eq((utilisateur.split("@")[0]).toUpperCase()))
																.fetch();
		if(roUtilisateursRecords.size()==1) return roUtilisateursRecords.get(0);
		else return null;		
	}
